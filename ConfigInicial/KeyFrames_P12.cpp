// Previo 12                                    Lopez Lopez Jose Rodrigo 	
// Fecha de entrega : 8 de mayo de 2026         320098446
...
// Variables clave para rotaciones y posiciones del cuerpo
float inclineDog = 0.0f; // Rotacion frontal (pitch)
float rollDog = 0.0f;    // Rotacion lateral (roll)
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Cuerpo: 3 rotaciones (Yaw, Pitch, Roll)
...
void DoMovement()
{
	...
	// Controles generales del cuerpo
	if (keys[GLFW_KEY_W]) dogPosZ += 0.01f;
	if (keys[GLFW_KEY_S]) dogPosZ -= 0.01f;
	if (keys[GLFW_KEY_A]) dogPosX += 0.01f;
	if (keys[GLFW_KEY_D]) dogPosX -= 0.01f;

	if (keys[GLFW_KEY_R]) dogPosY += 0.01f;
	if (keys[GLFW_KEY_F]) dogPosY -= 0.01f;

	if (keys[GLFW_KEY_Q]) rotDog += 1.0f;
	if (keys[GLFW_KEY_E]) rotDog -= 1.0f;

	if (keys[GLFW_KEY_T]) inclineDog += 1.0f;
	if (keys[GLFW_KEY_G]) inclineDog -= 1.0f;

	// Rotacion lateral
	if (keys[GLFW_KEY_B]) rollDog += 1.0f;
	if (keys[GLFW_KEY_N]) rollDog -= 1.0f;

	// Partes
	if (keys[GLFW_KEY_1]) head += 1.0f;
	if (keys[GLFW_KEY_2]) head -= 1.0f;
	if (keys[GLFW_KEY_U]) FLeftLeg += 1.0f;
	if (keys[GLFW_KEY_J]) FLeftLeg -= 1.0f;
	if (keys[GLFW_KEY_3]) FRightLeg += 1.0f;
	if (keys[GLFW_KEY_4]) FRightLeg -= 1.0f;

	if (keys[GLFW_KEY_Z]) RLegs += 1.0f;
	if (keys[GLFW_KEY_X]) RLegs -= 1.0f;

	if (keys[GLFW_KEY_C]) tail += 1.0f;
